...
class large_num {
	...
	int num_size;
		...
		large_num(const large_num &a){
			original_number = a.original_number;
			num_size = a.num_size;
			array_number = a.array_number;
		};
		...
		string convert_to_string(vector<short int> a){
			int i;
			string s;
			for(i=0;i<a.size();i++){
...
int main(){
	int i;
	string tempstring,tempstring2,s;
	vector<short int> v,v2,v3;
...
/*
	The addition function is a bit idiotic. It returns a string instead of a 
	large_num, you first need to convert the given string to a large_num before 
	doing any other stuff with it. thanks.
