...
/* Shortest Job Without Premption  */
...
void waiting_time(){
	...
	printf("Total\t%d\n",total_waiting_time);
	printf("Average::%.2f",(float)((float)total_waiting_time/n));
}
...
int main(){
	...
	scanf("%d",&n);
	printf("Enter %d lines with three integers each representing arrival\n and burst time,priority of each Process\n",n);
